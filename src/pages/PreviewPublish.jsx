...
export default function PreviewPublish() {
  ...
  if (isPublished) {
    return (
      <div className="success-banner" style={{ margin: '4rem auto', maxWidth: '600px', textAlign: 'center' }}>
        <div className="success-icon" style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: '64px', height: '64px', borderRadius: '50%', background: 'var(--primary-light)', color: 'var(--primary)', marginBottom: '24px' }}>
          <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
            <polyline points="20 6 9 17 4 12"></polyline>
          </svg>
        </div>
        <h2 style={{ fontSize: '24px', fontWeight: 600, color: 'var(--heading)', marginBottom: '8px' }}>Test Published!</h2>
        <p style={{ color: 'var(--body-text)', marginBottom: '24px', lineHeight: '1.6' }}>
  ...
  return (
    <div style={{ maxWidth: '850px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        ...
        <Card>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '24px', borderBottom: '1px solid var(--border)', paddingBottom: '24px', marginBottom: '24px' }}>
            <div>
              <Badge status={test.status || 'draft'} style={{ marginBottom: '8px' }} />
              <h1 className="page-title">{test.name}</h1>
              <p className="small-text text-muted" style={{ margin: '4px 0 0 0' }}>
                Subject: <strong style={{ color: 'var(--heading)' }}>{test.subject}</strong>
              </p>
            </div>
          ...
          <div className="form-grid-two-col" style={{ gridTemplateColumns: 'repeat(3, 1fr)', background: 'var(--primary-light)', padding: '24px', borderRadius: '12px', border: '1px solid rgba(91,92,235,0.15)' }}>
            <div>
              <span className="caption text-muted" style={{ textTransform: 'uppercase', fontWeight: 600, display: 'block', marginBottom: '4px' }}>Parameters</span>
              <div className="small-text" style={{ fontWeight: 600, color: 'var(--heading)', lineHeight: '1.6' }}>
            ...
            <div>
              <span className="caption text-muted" style={{ textTransform: 'uppercase', fontWeight: 600, display: 'block', marginBottom: '4px' }}>Marking Scheme</span>
              <div className="small-text" style={{ fontWeight: 600, color: 'var(--heading)', lineHeight: '1.6' }}>
                Correct: <span style={{ color: 'var(--success)' }}>+{test.correct_marks}</span> <br/>
                Incorrect: <span style={{ color: 'var(--danger)' }}>{test.wrong_marks}</span> <br/>
                Unattempted: <span style={{ color: 'var(--body-text)' }}>{test.unattempt_marks}</span>
              </div>
            </div>

            <div>
              <span className="caption text-muted" style={{ textTransform: 'uppercase', fontWeight: 600, display: 'block', marginBottom: '4px' }}>Totals Summary</span>
              <div className="small-text" style={{ fontWeight: 600, color: 'var(--heading)', lineHeight: '1.6' }}>
          ...
          {test.topics && test.topics.length > 0 && (
            <div style={{ marginTop: '24px' }}>
              <span className="caption text-muted" style={{ fontWeight: 500, display: 'block', marginBottom: '8px' }}>
                Targeted Topics & Sub-topics
              </span>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                {test.topics.map((t, i) => (
                  <Badge key={`t-${i}`} status="draft" style={{ background: 'var(--primary-light)', color: 'var(--primary)', border: '1px solid rgba(91,92,235,0.1)' }}>
                    {t}
                  </Badge>
                ))}
                {test.sub_topics && test.sub_topics.map((st, i) => (
                  <Badge key={`st-${i}`} status="draft" style={{ background: 'rgba(124, 58, 237, 0.1)', color: 'var(--primary)', border: '1px solid rgba(124, 58, 237, 0.1)' }}>
                    {st}
                  </Badge>
                ))}
              </div>
        ...
        <div>
          <h2 className="section-title mb-4">Questions Preview ({questions.length})</h2>
            ...
            <div>
              {questions.map((q, index) => (
                <Card key={q.id || index} className="mb-4">
                  ...
                  <div style={{ fontSize: '16px', fontWeight: 500, marginBottom: '20px', color: 'var(--heading)', whiteSpace: 'pre-wrap' }}>
                    {q.question}
                  </div>

                  {q.media_url && (
                    <div style={{ marginBottom: '20px', border: '1px solid var(--border)', borderRadius: '12px', overflow: 'hidden', maxWidth: '350px' }}>
                      <img 
                        src={q.media_url} 
                  ...
                  {q.explanation && (
                    <div style={{ marginTop: '20px', background: 'var(--background)', border: '1px solid var(--border)', borderRadius: '12px', padding: '16px' }}>
                      <strong style={{ fontSize: '13px', color: 'var(--primary)', display: 'block', marginBottom: '4px' }}>Solution Explanation:</strong>
                      <span style={{ fontSize: '14px', color: 'var(--body-text)' }}>{q.explanation}</span>
                    </div>
                  )}
                </Card>
              ))}
